...
# Capabilities that the sandbox never legitimately needs.
# NET_ADMIN / NET_RAW are added back explicitly for nmap raw sockets;
# everything else in this list is pure attack surface that a pentest agent
# running inside Docker has no reason to hold.
_HARDENED_CAP_DROP = [
    "AUDIT_WRITE",
    "CHOWN",
    "DAC_OVERRIDE",
...
def _apply_security_hardening(create_kwargs: dict[str, Any]) -> None:
    """Drop unnecessary Linux capabilities and set security options.

    Enabled by default unless STRIX_SANDBOX_SKIP_HARDENING=1 is set (useful
    when running inside a Docker-in-Docker environment that rejects cap_drop).

    The cap_add list (NET_ADMIN/NET_RAW for nmap raw sockets) is preserved —
    cap_drop is applied first, then cap_add re-grants only what's listed.
    Linux evaluates the effective set as: (initial & ~cap_drop) | cap_add.

    Reference: CIS Docker Benchmark v1.6 §5.3; NIST SP 800-190 §4.4.
    """
    if os.environ.get("STRIX_SANDBOX_SKIP_HARDENING", "").strip() in ("1", "true", "yes"):
        logger.debug("Sandbox security hardening disabled via STRIX_SANDBOX_SKIP_HARDENING")
        return

    # Drop capabilities not in the explicit allow-list (cap_add wins over cap_drop
    # for the same capability, so NET_ADMIN/NET_RAW remain effective).
    cap_add: list[str] = create_kwargs.get("cap_add") or []
    existing_drop: list[str] = create_kwargs.get("cap_drop") or []
    to_drop = [c for c in _HARDENED_CAP_DROP if c not in cap_add and c not in existing_drop]
    if to_drop:
        create_kwargs["cap_drop"] = existing_drop + to_drop

    # no-new-privileges: prevents setuid/setgid programs from escalating.
    sec_opts: list[str] = list(create_kwargs.get("security_opt") or [])
    if not any("no-new-privileges" in s for s in sec_opts):
        sec_opts.append("no-new-privileges:true")
    create_kwargs["security_opt"] = sec_opts

    # Read-only root filesystem where possible.  Skipped when FUSE or
    # SYS_ADMIN is in cap_add (those mounts need a writable fs layer).
    if not any(c in cap_add for c in ("SYS_ADMIN", "FUSE")):
        read_only = os.environ.get("STRIX_SANDBOX_READ_ONLY_ROOT", "").strip()
        if read_only in ("1", "true", "yes"):
            create_kwargs["read_only"] = True

    logger.debug(
        "Security hardening applied: cap_drop=%d security_opt=%s",
        len(to_drop),
        create_kwargs.get("security_opt"),
    )
...
class StrixDockerSandboxClient(DockerSandboxClient):
    ...
    async def _create_container(
        ...
        *,
        ...
        _apply_security_hardening(create_kwargs)
