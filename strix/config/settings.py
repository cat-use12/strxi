...
class LlmSettings(BaseSettings):
    ...
    refusal_fallback_model: str | None = Field(
        default=None,
        alias="STRIX_LLM_REFUSAL_FALLBACK",
        description=(
            "Fallback model for safety classifier refusals (stop_reason='refusal', category='cyber'). "
            "Claude Fable 5/5.1 and Opus 5 have a hard classifier layer — benign security work can "
            "trigger it. Set to a less-restricted model such as 'litellm/claude-opus-4-8' or "
            "'litellm/claude-sonnet-4-6' to auto-retry refused requests. "
            "See Anthropic docs: build-with-claude/refusals-and-fallback"
        ),
    )
...
class FastModelSettings(BaseSettings):
    """Optional fast/cheap model for quick recon tasks (STRIX_LLM_FAST).

    Falls back to the primary LLM when unset.
    Ideal for: web crawling, port scan summaries, quick lookups.

    Example: STRIX_LLM_FAST=groq/llama-3.1-8b-instant
    """

    model_config = _BASE_CONFIG

    model: str | None = Field(default=None, alias="STRIX_LLM_FAST")
    api_key: str | None = Field(default=None, alias="FAST_LLM_API_KEY", repr=False)
    api_base: str | None = Field(default=None, alias="FAST_LLM_API_BASE")
    extra_headers: dict[str, str] | None = Field(
        default=None,
        alias="FAST_LLM_EXTRA_HEADERS",
        repr=False,
    )


class CodeModelSettings(BaseSettings):
    """Optional code-specialized model for exploit/code analysis (STRIX_LLM_CODE).

    Falls back to the primary LLM when unset.
    Ideal for: PoC code review, dependency CVE triage, source code analysis.

    Example: STRIX_LLM_CODE=ollama/qwen2.5-coder:14b
    """

    model_config = _BASE_CONFIG

    model: str | None = Field(default=None, alias="STRIX_LLM_CODE")
    api_key: str | None = Field(default=None, alias="CODE_LLM_API_KEY", repr=False)
    api_base: str | None = Field(default=None, alias="CODE_LLM_API_BASE")
    extra_headers: dict[str, str] | None = Field(
        default=None,
        alias="CODE_LLM_EXTRA_HEADERS",
        repr=False,
    )
...
class Settings(BaseSettings):
    ...
    code_model: CodeModelSettings = Field(default_factory=CodeModelSettings)
