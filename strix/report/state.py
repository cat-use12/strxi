...
from strix.report.executive_summary import write_executive_summary
...
class ReportState:
    ...
    # Valid transitions for the remediation state machine.
    # OPEN is the initial state for every new finding.
    _REMEDIATION_TRANSITIONS: dict[str, list[str]] = {
        "open": ["in_progress", "wont_fix"],
        "in_progress": ["open", "pending_retest", "wont_fix"],
        "pending_retest": ["open", "verified_fixed", "in_progress"],
        "verified_fixed": ["open"],
        "wont_fix": ["open"],
    }
    ...
    def transition_remediation_status(
        ...
        *,
        ...
        """Advance the remediation lifecycle state of a finding.

        Returns a dict with ``success`` and diagnostic fields.
        """
        report = next(
        ...
        current = str(report.get("remediation_status") or "open").lower()
        allowed = self._REMEDIATION_TRANSITIONS.get(current, [])
        ...
        if updated_by and updated_by.strip():
            event["updated_by"] = updated_by.strip()

        history = list(report.get("remediation_history") or [])
        history.append(event)

        report["remediation_status"] = new_status
        report["remediation_history"] = history
        report["remediation_updated_at"] = timestamp

        logger.info(
            "Remediation status for %s: %s → %s",
            report_id,
            current,
            new_status,
        )
    ...
    def get_remediation_summary(self) -> dict[str, Any]:
        """Return a count of findings per remediation status."""
        counts: dict[str, int] = {
            "open": 0,
            "in_progress": 0,
            "pending_retest": 0,
        ...
        for r in self.vulnerability_reports:
            status = str(r.get("remediation_status") or "open").lower()
        ...
        return {
            "total": total,
            "counts": counts,
            "remediation_rate": round(fixed / total * 100, 1) if total else 0.0,
        }
    ...
    def save_run_data(self, mark_complete: bool = False, status: str | None = None) -> None:
        ...
        # Persist findings to the cross-scan knowledge base on completion
        if mark_complete and self.vulnerability_reports:
            try:
                from strix.core.scan_memory import record_findings
                _target_str = ""
                for _t in (self.run_record.get("targets_info") or []):
                    if isinstance(_t, dict):
                        _target_str = _t.get("target") or _t.get("url") or ""
                    else:
                        _target_str = str(_t)
                    if _target_str:
                        break
                ...
                logger.exception("scan_memory persist failed (non-fatal)")
...
"""
    ...
    def _save_artifacts(self) -> None:
        ...
        try:
            ...
            # Executive summary (non-technical management report)
            try:
                _targets: list[str] = []
                for _t in (self.run_record.get("targets_info") or []):
                    if isinstance(_t, dict):
                        _v = _t.get("target") or _t.get("url") or ""
                    else:
                        _v = str(_t)
                    if _v:
                        _targets.append(_v)
                ...
                logger.exception("executive_summary.md write failed (non-fatal)")
