...
from strix.core.prompt_injection_guard import scan_web_content
...
def _do_search(query: str) -> dict[str, Any]:
    ...
    # Prompt injection guard: scan content returned by the target's web presence
    # before it enters the agent's context (ported from NousResearch/hermes-agent).
    warning = scan_web_content(outcome, source_url=query, scope="context")
    if warning:
        logger.warning("Prompt injection detected in web search results: %s", warning)
        return {
            "success": True,
            "query": query,
            "provider": provider,
            "content": outcome,
            "security_warning": str(warning),
        }
