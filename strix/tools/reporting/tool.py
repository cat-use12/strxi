...
async def _do_create(
    *,
    ...
    # Safety-validate the PoC script before accepting the report.
    # This blocks destructive payloads (filesystem wipes, fork bombs) while
    # leaving all legitimate offensive security tools and payloads untouched.
    if poc_script_code and poc_script_code.strip():
        from strix.tools.exploit_validator import validate_poc_script

        _val = validate_poc_script(poc_script_code, language="auto")
        if not _val.safe:
            errors.append(f"PoC script safety check failed: {_val.reason}")
