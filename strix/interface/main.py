...
def main() -> None:
    ...
    # `strix retest …` — manage remediation lifecycle for findings and
    # re-verify fixed vulnerabilities without a full re-scan.
    if len(sys.argv) > 1 and sys.argv[1] == "retest":
        from strix.interface.retest_cli import run_retest

        sys.exit(run_retest(sys.argv[2:]))

    # `strix auth-scope …` — manage the authorization scope file.
    if len(sys.argv) > 1 and sys.argv[1] == "auth-scope":
        from strix.interface.auth_scope_cli import run_auth_scope

        sys.exit(run_auth_scope(sys.argv[2:]))
