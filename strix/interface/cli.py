    ...
    try:
        ...
        with Live(
            ...
            try:
                ...
                _auth_scope_path = None
                _raw_auth_scope = getattr(args, "auth_scope_file", None)
                if _raw_auth_scope:
                    from pathlib import Path as _Path

                    _auth_scope_path = _Path(_raw_auth_scope).expanduser()
                # Build extra system prompt context (bug bounty mode, etc.)
                _extra_ctx: dict[str, Any] | None = None
                _bb = getattr(args, "bug_bounty", None)
                if _bb:
                    _extra_ctx = {
                        "bug_bounty_program": _bb,
                        "bug_bounty_platform": getattr(args, "bug_bounty_platform", None) or "HackerOne / Bugcrowd",
                    }
                ...
                await run_strix_scan(
                    ...
                    auth_scope_file=_auth_scope_path,
                    auth_enforce=bool(getattr(args, "auth_enforce", False)),
                    extra_system_prompt_context=_extra_ctx,
                )
            finally:
                stop_updates.set()
