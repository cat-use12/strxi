...
Strix Cloud:
  ...
  Try Strix Enterprise             https://strix.ai/demo
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        help=(
            "Path to an authorized-scopes.yaml file that declares which targets you "
            "have permission to test. Every target must match an entry in this file. "
            "Defaults to ~/.strix/authorized-scopes.yaml when that file exists. "
            "Set STRIX_AUTH_ENFORCE=1 to treat unmatched targets as a hard error "
            "instead of a warning. Generate a starter file with: "
            "'strix auth-scope init --target <url> --authorized-by \"Name <email>\"'."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Treat authorization failures as hard errors (abort the scan). "
            "By default an unmatched target emits a warning and continues. "
            "Equivalent to setting STRIX_AUTH_ENFORCE=1."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Declare this scan as a bug bounty engagement. "
            "Provide the program name (e.g. 'HackerOne: TargetCorp' or 'Bugcrowd: AcmeCorp'). "
            "This adds explicit bug bounty authorization context to the agent's system prompt, "
            "which reduces unnecessary refusals on standard security testing techniques. "
            "Example: --bug-bounty 'HackerOne: targetcorp' "
            "Equivalent to setting STRIX_BUG_BOUNTY env var."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Bug bounty platform name (e.g. 'HackerOne', 'Bugcrowd', 'Intigriti', 'YesWeHack'). "
            "Used together with --bug-bounty. Defaults to 'HackerOne / Bugcrowd'."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Load a named scan profile from ~/.strix/profiles/. "
            "Built-in profiles: normal, stealth, silent, aggressive, api-only, recon, quick. "
            "Profiles set stealth level, max turns, and custom instructions. "
            "Example: --profile stealth"
        ),
    )
    ...
    # Resolve --bug-bounty from flag or env var.
    if not args.bug_bounty:
        args.bug_bounty = os.environ.get("STRIX_BUG_BOUNTY", "").strip() or None
    if not args.bug_bounty_platform:
        args.bug_bounty_platform = os.environ.get("STRIX_BUG_BOUNTY_PLATFORM", "").strip() or None

    # Resolve --profile from flag or env var and apply it.
    if not args.profile:
        args.profile = os.environ.get("STRIX_PROFILE", "").strip() or None
    if args.profile:
        try:
            from strix.config.profiles import load_profile, apply_profile
            profile = load_profile(args.profile)
            apply_profile(profile)
            # If profile provides an instruction and none was given, use it.
            profile_instruction = profile.get("instruction") or os.environ.get("STRIX_PROFILE_INSTRUCTION", "")
            if profile_instruction and not args.instruction and not args.instruction_file:
                args.instruction = profile_instruction
        except Exception as _profile_err:
            import warnings
            warnings.warn(f"Could not load profile '{args.profile}': {_profile_err}", stacklevel=2)
