...
async def run_strix_scan(
    *,
    ...
    auth_scope_file: Path | None = None,
    ...
    # --- Authorization gate ------------------------------------------------
    # Check that every target in scan_config is covered by an authorized scope
    # before any sandbox is created or any LLM call is made.
    _raw_targets: list[str] = []
    for t in scan_config.get("targets_info", []) or []:
        if isinstance(t, dict):
            raw = t.get("target") or t.get("url") or ""
        else:
            raw = str(t)
        if raw.strip():
            _raw_targets.append(raw.strip())

    if _raw_targets:
        from strix.core.authorization import AuthorizationError, enforce_authorization

        try:
            _auth_results = enforce_authorization(
                _raw_targets,
                scope_file=auth_scope_file,
                enforce=auth_enforce,
            )
            for _ar in _auth_results:
                if _ar.authorized and _ar.matched_scope:
                    logger.info(
                        "[AUTH] Target '%s' authorized by %s (scope: %s)",
                        _ar.target,
                        _ar.matched_scope.authorized_by,
                        _ar.matched_scope.scope_notes or "—",
                    )
        except AuthorizationError as _auth_err:
            raise RuntimeError(f"[AUTHORIZATION] Scan aborted: {_auth_err}") from _auth_err
    # -----------------------------------------------------------------------
